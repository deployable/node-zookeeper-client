...
class TypeBytes {
  ...
  static vector (values){
    let size = 4
    debug('values', values)
    if (isArray(values)) forEach(values, item => size += byteLength(type, item))
    return size
  }
...
class Record {
  ...
  static typeByteLength(type, value) {
    ...
    if (fn = TypeBytes[type.type]) return fn(value)
  ...
  set data(val){
    this._data = tbuffer(val)
  }

  get acl(){ return this._acl }
  set acl(val){
    this._acl = tvector(val, 'data.ACL')
  }

  get flags(){ return this._flags}
  set flags(val){
    this._flags = tint(val)
  }

  get version()   { return this._version }
  set version(val){ this._version = tint(val) }
  ...
  set zxid(val){ this._zxid = tint(val) }

  get err(){ return this._err}
  set err(val){ this._err = tint(val) }
  ...
  byteLength() {
    ...
    forEach(this._order, attribute => {
      this.debug('%s %s byte length attribute: %s', this.constructor.name, this._uid, attribute, this[attribute])
      size += Record.typeByteLength(myTypes[attribute].type, this[attribute])
    })
  ...
  serialize(buffer, offset) {
    ...
    let myTypes = this._attributes
    this.debug('%s serialize %s', this.constructor.name, this._order)
  ...
  deserialize(buffer, offset) {
    ...
    this.debug('%s deserialize %s', this.constructor.name, this._order)
    forEach(this._order, attribute => {
