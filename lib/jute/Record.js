...
class Record {
  ...
  byteLength() {
    ...
    this._order.forEach(attribute => {
      debug('%s[%s] byte length attribute:%s val:%s size:%s', 
  ...
  deserialize(buffer, offset) {
    ...
    let bytesRead = 0
    this._order.forEach(attribute => {
      debug('attribute', attribute, this[attribute] && this[attribute].value )
  ...
  // Convert the object to a JS representation, rather than full Jute types
  toJS(){
    let o = {}
    this._order.forEach(attribute => {
      let att = this[attribute]
      if ( att === undefined || att === null ){
        return o[attribute] = att
      }
      if ( att.toJS ) return o[attribute] = att.toJS()

      if ( att.value ) return o[attribute] = att.value

      debug('typeof atttribute %s is %s', attribute, typeof attribute)
      return o[attribute] = att

      // Otherwise its a plain value, which shouldn't be there
      throw new JuteError('toJS() failed. We shouldn\'t get here')
    })
    return o
  }
