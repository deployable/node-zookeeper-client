...
class TransactionRequest {
  ...
  serialize(buffer, offset){
    if( !Buffer.isBuffer(buffer) )
      throw new ZkError('buffer must an instance of Node.js Buffer class.')

    if ( offset >= 0 && offset < buffer.length )
      throw new ZkError('offset: ' + offset + ' is out of buffer range.')

    var size = this.byteLength()

    if ( offset + size > buffer.length )
      throw new ZkError('buffer does not have enough space.')
