const debug = require('debug')('dply:node-zookeeper-client:jute:Deserialize')
...
class Deserialize {
  ...
  static byType( type, buffer, offset ){
    let subtype
    debug('deserialize by type %s', type, buffer, offset)
    
    // Normal types
    if ( this[type] ) return this[type](buffer, offset)
    
    // Vectors are a bit special
    if ( type.startsWith('vector<') )
      return this.vector(this.vectorType(type), buffer, offset)
    
    // So is data
    if ( type.startsWith('data.') )
      return this.data(this.dataType(type), buffer, offset)
