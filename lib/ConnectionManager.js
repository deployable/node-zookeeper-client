...
const ConnectionStringParser = require('./ConnectionStringParser.js')
const WatcherManager = require('./WatcherManager.js')
const PacketQueue = require('./PacketQueue.js')
...
class ConnectionManager extends events.EventEmitter {
  ...
  cleanupPendingQueue(errorCode) {
    ...
    while (pendingPacket) {
      if (pendingPacket.callback) pendingPacket.callback(Exception.create(errorCode))
      ...
      if (connectResponse.timeOut <= 0) {
        ...
        default:
            ...
            if (pendingPacket.request.header.xid !== responseHeader.xid) {
              this.debug('Xid out of order. Got xid:%s wanted:%s', responseHeader.xid, pendingPacket.request.header.xid)
                    this.debug('destroying socket')
              this.socket.destroy() // this will trigger reconnect
              return
            }
            ...
            if (responseHeader.err === 0) {
              ...
              if (responsePayload) {
                if (self.chrootPath) responsePayload.setChrootPath(self.chrootPath)
