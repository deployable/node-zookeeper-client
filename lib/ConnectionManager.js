...
class ConnectionManager extends events.EventEmitter {
  ...
  constructor (connectionString, options = {}, stateListener) {
    ...
    this.servers = this.connectionString.getServers()
    this.chrootPath = this.connectionString.getChrootPath()
  ...
  cleanupPendingQueue(errorCode) {
    ...
    while (pending_pkt = this.pendingQueue.shift()) {
      ...
      if (pending_pkt.reject) {
        pending_pkt.reject(ZkException.create(errorCode))
      }
      else {
  ...
  onSocketClosed(error) {
    ...
    switch (this.state) {
      ...
      case STATE.CLOSING:
          errorCode = ZkException.CONNECTION_LOSS
          retry = false
          break
    
      case STATE.SESSION_EXPIRED:
          errorCode = ZkException.SESSION_EXPIRED
          retry = false
          break
      
      case STATE.AUTH_FAILED:
          errorCode = ZkException.AUTH_FAILED
          retry = false
          break
...
//    response,
    ...
    switch (response_header.xid) {
      ...
      case jute.XID_AUTHENTICATION:
        ...
        if (response_header.err === ZkException.AUTH_FAILED) {
    ...
    if (response_header.err.value !== 0) {
      let err = ZkException.create(response_header.err.value)
    ...
    return new Promise((resolve, reject)=>{
      ...
      switch (this.state) {
        ...
        case STATE.CLOSING:
            ...
            return reject(ZkException.create(ZkException.CONNECTION_LOSS))

        case STATE.CLOSED:
            return reject(ZkException.create(ZkException.CONNECTION_LOSS))

        case STATE.SESSION_EXPIRED:
            return reject(ZkException.create(ZkException.SESSION_EXPIRED))

        case STATE.AUTHENTICATION_FAILED:
            return reject(ZkException.create(ZkException.AUTH_FAILED))
