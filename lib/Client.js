    ...
    return new Promise((resolve) => {
      ...
      return resolve(this.connManager.attempt(
        this.connManager.queue(request).then(response =>{
          ...
          if (watcher) {
            if (existence) this.connManager.registerDataWatcher(zkpath, watcher)
            else this.connManager.registerExistenceWatcher(zkpath, watcher)
          }
