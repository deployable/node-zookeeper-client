...
module.exports = class Client extends events.EventEmitter {
  ...
  setACL( zkpath, options = {} ){
    return new Promise((resolve) => {
      ...
      let payload = new jute.protocol.SetACLRequest(zkpath, undefined, version)
        .addACLArray(acls)
  ...
  getACL ( zkpath ) {
    return new Promise((resolve) => {
      ...
      let payload = new jute.protocol.GetACLRequest(zkpath)
      let request = new jute.Request(header, payload)
    ...
    return new Promise((resolve) => {
      ...
      return resolve(this.connManager.attempt(
        this.connManager.queue(request).then(response =>{
          ...
          let existence = (response.header.err.value === Exception.OK)
          if (watcher) {
        ...
        .catch(error => {
          if ( !error.getCode || error.getCode() !== Exception.NO_NODE) throw error
          return null
        })
      ))
    })
    ...
    return new Promise((resolve) => {
      ...
      let result = Promise.each(nodes, node => {
          ...
          return this.create(currentPath, { data: data, acls: acls, mode: mode })
            .catch(error => {
              if ( error.getCode() === Exception.NODE_EXISTS ) return currentPath
              throw error
            })
        })
        .then(results => {
